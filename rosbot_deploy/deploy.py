...
def create_deployment(robomaker, fleet_arn, app_arn, app_ver):
    ...
    launch_file = 'await_commands.launch.py'
...
def start_deployment(user_fleet_name, user_robot_name, s3bucket):
    ...
    app_key_prefix = 'cloudwatchsample/robot_ws/bundle'
    print('Clean fleet')
    ...
    if rosbot_arn:
        ...
        return None
    application_arn = get_robot_application_arn(robomaker_client, 'CloudWatchSample')
    if application_arn:
        add_armhf_to_app(robomaker_client, application_arn, s3bucket, app_key_prefix)
    else:
        application_arn = create_robot_app(robomaker_client, 'CloudWatchSample', s3bucket, app_key_prefix)
